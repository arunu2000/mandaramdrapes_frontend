...
//           </div>
//           <ul className="flex justify-center gap-10 text-sm items-center font-medium">
//             {/* <li to="/" className="hover:text-gray-900 transition-colors">
//               Home
//             </li> */}
...
//           <div
//             onClick={() => {
//               token && role === "user" ? navigate("/") : navigate("/login");
//             }}
//           >
//             {token && role === "user" ? (
//               <img
...
// // --- Utility Icons --- (Keeping your icons for consistency, though not used in Menu)
// const ChevronLeftIcon = (props) => (
//     <svg
//       {...props}
//       xmlns="http://www.w3.org/2000/svg"
...
// // Example product data (kept as-is)
// const products = [
//     {
//         id: 1,
//         name: "Zip Tote Basket",
...
//     // Placeholder for User Name (You would typically fetch this from an API or store it in localStorage/context)
//     const userName = "Unaiz Noushad"; 

//     // 1. Function to handle the logout logic
//     const handleLogout = () => {
//         // Clear authentication data
//         localStorage.removeItem("token");
//         localStorage.removeItem("role"); 
//         // Force reload or navigate to ensure state is completely reset and user sees log-in prompt
//         navigate("/"); 
//         // Optional: window.location.reload(); 
//     };

//     // 2. Logic to handle the original Avatar/Login click behavior
//     const handleAvatarClick = () => {
//       // If the user is NOT logged in or is not a 'user' role, navigate to login
//       if (!token || role !== "user") {
//         navigate("/login");
//       }
//       // If they ARE logged in as a 'user', the Menu component handles the click to open the dropdown.
//       // We don't need an explicit action here as the MenuButton takes over.
//     };
...
//                     {/* 3. Replaced the static div with Headless UI Menu */}
//                     <Menu as="div" className="relative">
//                         <MenuButton 
//                             className="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#5e785a] rounded-full"
//                             onClick={handleAvatarClick} // Only navigates if not logged in
//                         >
//                             {token && role === "user" ? (
//                                 <img
//                                     alt="User Avatar"
...
//                                 />
//                             ) : (
//                                 <button className="text-gray-600 hover:text-[#5e785a] font-medium transition-colors">Login</button>
//                             )}
//                         </MenuButton>

//                         {/* Only show MenuItems if the user is logged in as 'user' */}
//                         {token && role === "user" && (
//                             <MenuItems
//                                 transition
//                                 anchor="bottom end"
...
//                                     <MenuItem>
//                                         <Link
//                                             to="/profile" // Link to a user profile page (optional)
...
//             {/* Rest of your component (Hero, Category, Product, Footer) remains unchanged */}
//             {/* ... (Hero Section) ... */}
//             <div className="mt-16 relative w-full">
//                 <Carousel autoPlay infiniteLoop showThumbs={true}>
//                     <div>
...
const Customerdashboard = () => {
    ...
    if (token && role === 'user') {
      const fetchProfile = async () => {
        ...
        try {
          ...
          if (!response.ok) {
            // Handle unauthorized or bad response
          ...
          // Optional: clear token and force logout if profile fetch fails
          // handleLogout(false); 
        } finally {
          setIsLoading(false);
        }
      ...
      setIsLoading(false);
      setUserProfile(null); // Clear profile if token is missing
    }
  }, [token, role]); // Re-run effect if token/role changes
  ...
  const handleLogout = (redirect = true) => {
    ...
    localStorage.removeItem("token");
    localStorage.removeItem("role"); 
    setUserProfile(null); // Clear client-side state
    if (redirect) {
        navigate("/login"); // Redirect to login page or homepage
    }
  };

  // --- Login/Avatar Click Handler ---
  const handleAvatarClick = () => {
      ...
      <nav className="fixed top-0 left-0 w-full bg-white shadow-md z-50">
        <div className="flex justify-around items-center text-[#5e785a] p-3">
          ...
          <Menu as="div" className="relative">
            ...
            {/* Show MenuItems ONLY if logged in and profile is loaded */}
            {token && role === "user" && userProfile && (
              <MenuItems
                transition
                anchor="bottom end"
                className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 transition focus:outline-none data-[closed]:scale-95 data-[closed]:transform data-[closed]:opacity-0 data-[enter]:duration-100 data-[leave]:duration-75 data-[enter]:ease-out data-[leave]:ease-in z-50"
              >
                <div className="px-4 py-3">
                  <p className="text-sm font-medium text-gray-900">Signed in as:</p>
                  {/* Display the REAL user name/email from state */}
                  <p className="truncate text-sm font-semibold text-[#5e785a]">
                ...
                <div className="py-1">
                  <MenuItem>
                    <Link
                      to="/profile" // You might want to create a profile view page
                      className="block px-4 py-2 text-sm text-gray-700 data-[focus]:bg-gray-100 transition-colors"
                    >
                      My Profile
                    </Link>
                ...
                <div className="py-1">
                  <MenuItem>
                    <button
                      onClick={() => handleLogout()} // Trigger the logout handler
                      className="w-full text-left block px-4 py-2 text-sm text-red-600 data-[focus]:bg-gray-100 transition-colors"
                    >
                      Logout
                    </button>
            ...
            {token && role === "user" && isLoading && (
                <div className="text-sm text-gray-500 absolute right-0 mt-2 p-2 bg-white rounded-md shadow">Loading...</div>
            )}
          </Menu>
