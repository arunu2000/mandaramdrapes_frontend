...
//                 {/* <a href="/Customer" className="mt-4 inline-block text-indigo-600 hover:text-indigo-500 font-medium">
//                     Start Shopping
//                 </a> */}
//             </div>
//         );
...
import { CheckIcon, ClockIcon, QuestionMarkCircleIcon, XMarkIcon } from '@heroicons/react/20/solid'
// NOTE: BACKEND_BASE_URL is generally not needed here if context handles image paths
// const BACKEND_BASE_URL = "http://192.168.29.217:5000/"; 

export default function Cartpage() {
    const navigate = useNavigate();
    // 1. Get the cart state and functions, including loading and fetchCart
    const { 
        cartItems, 
        loading, 
        error, // Catch error state
        fetchCart, // For manual refresh if needed
        removeFromCart, 
        updateQuantity, 
        cartTotal,
        placeOrder
    ...
    // 2. Show Loader while fetching (This is the solution to your request)
    if (loading) {
        return (
    ...
    if (error) {
        return <div className="text-center py-20 text-red-600">⚠️ {error}</div>;
    }

    // 4. Handle Empty Cart (after loading is complete)
    if (cartItems.length === 0) {
    ...
    const handleQuantityChange = (item, event) => {
        const newQuantity = parseInt(event.target.value);
        if (newQuantity >= 1) {
            // This now calls the CONTEXT function which sends the update to the backend
    ...
    const handlePlaceOrder = async (e) => {
        ...
        try {
            await placeOrder();
            // Redirect user to the Orders page or a confirmation page
            navigate('/my-orders'); 
            alert("Order Placed Successfully!");
        } catch (error) {
            // Error handled in context, but you can display a local message here
            alert("Error placing order: " + (error.message || "Please check console."));
        }
    };
    
    return (
        // ... (The rest of your existing Cartpage component) ...
        // <div className="bg-white">
        //     <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
        //         <h1 className="text-3xl font-bold tracking-tight text-gray-900">Shopping Cart</h1>
        ...
        //                             <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
        //                                 <img
        //                                     // The image path needs to be correct (using item.image directly if full path comes from backend)
      ...
      <div className="mx-auto max-w-2xl px-4 pt-16 pb-24 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        <form className="mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
          ...
          >
            ...
            <div className="mt-6">
             <button
                                // ⚠️ Change type to "button" and use onClick ⚠️
                                type="button" 
